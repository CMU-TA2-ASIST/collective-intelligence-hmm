...
def visualize_O(hmm_data, savename=''):
    # for group in group_no_to_hmm:
#     hmm = group_no_to_hmm[group]
    A = hmm_data['new_A']
    O = hmm_data['new_O']
    state_id_to_state = hmm_data['state_id_to_state']
    ...
    for i in range(plot_matrix.shape[0]):
        sum_over_row = sum(O[i,:])
        plot_matrix[i, :] = O[i,:]/sum_over_row

    ax.matshow(plot_matrix, cmap=plt.cm.Blues)

    for i in range(plot_matrix.shape[1]):
        for j in range(plot_matrix.shape[0]):
            c = np.round(plot_matrix[j,i], 2)
            if c != 0:
                ax.text(i, j, str(c), va='center', ha='center')


    #     plt.colorbar()
    # title_string = group_no_to_title[group]
    #     plt.suptitle(title_string, y=0.9, fontsize=18)
    plt.title(f'State-Obs Transition Matrix', fontsize=18)

    y = [0, 1, 2, 3, 4, 5]
    labels = ['State 1: Poor', 'State 2', 'State 3', 'State 4', 'State 5', 'State 6: Good']
    plt.yticks(y, labels, rotation='horizontal')

    x = range(plot_matrix.shape[1])
    labels = [state_id_to_state[i] for i in x]
    plt.xticks(x, labels, rotation='vertical')


    plt.xlabel("Observation Index", fontsize=15)
    plt.ylabel("Current Hidden State (CI)", fontsize=15)
    plt.savefig(f'hmm_data/{savename}_O_matrix.png')
    plt.close()


def visualize_sparsities(A, O, savename=''):
    plt.close('all')
    plt.set_cmap('Reds')
    ...
    plt.colorbar()
    plt.title('Sparsity of State-State Transition Matrix')

    y = [0, 1, 2, 3, 4, 5]
    labels = ['State 1: Low CI', 'State 2', 'State 3', 'State 4', 'State 5', 'State 6: Good']
    plt.yticks(y, labels, rotation='horizontal')
    ...
    plt.savefig(f'hmm_data/{savename}_A_matrix.png')
    plt.close()
